...
static struct clk_vdd_class *gpu_cc_yupik_regulators[] = {
	&vdd_cx,
	&vdd_mx,
};
...
static struct clk_regmap *gpu_cc_yupik_clocks[] = {
...
static const struct of_device_id gpu_cc_yupik_match_table[] = {
	{ .compatible = "qcom,yupik-gpucc" },
	{ }
};
MODULE_DEVICE_TABLE(of, gpu_cc_yupik_match_table);

static int gpu_cc_yupik_probe(struct platform_device *pdev)
{
	struct regmap *regmap;
	int ret;

	regmap = qcom_cc_map(pdev, &gpu_cc_yupik_desc);
	if (IS_ERR(regmap))
...
{
	qcom_cc_sync_state(dev, &gpu_cc_yupik_desc);
}

static struct platform_driver gpu_cc_yupik_driver = {
	.probe = gpu_cc_yupik_probe,
	.driver = {
		.name = "gpu_cc-yupik",
...
static int __init gpu_cc_yupik_init(void)
{
	return platform_driver_register(&gpu_cc_yupik_driver);
}
subsys_initcall(gpu_cc_yupik_init);

static void __exit gpu_cc_yupik_exit(void)
{
	platform_driver_unregister(&gpu_cc_yupik_driver);
}
module_exit(gpu_cc_yupik_exit);

MODULE_DESCRIPTION("QTI GPU_CC YUPIK Driver");
